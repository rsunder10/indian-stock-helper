...
def refresh(args) -> int:
    ...
    if args.repo_rate is not None:
        pack["prev_repo_rate"] = args.prev_repo_rate if args.prev_repo_rate is not None else pack.get("repo_rate")
    ...
    if args.cpi_resource:
        ...
        if not api_key:
            print("  ! --cpi-resource given but no BUDGET_API_KEY in env; skipping CPI fetch", file=sys.stderr)
        else:
            try:
                ...
                return 1
            if cpi is None:
                print("  ✗ CPI fetch returned no parseable value; check --field-cpi / resource", file=sys.stderr)
                return 1
            pack["cpi_yoy"] = cpi
            changed.append(f"CPI={cpi}% (fetched)")

    if not changed:
        print("  ! nothing to change — pass --repo-rate/--stance/--cpi or --cpi-resource.", file=sys.stderr)
        return 1
    ...
    pack["regime"] = "easing" if (repo is not None and prev and repo < prev) else \
        "tightening" if (repo is not None and prev and repo > prev) else "neutral"
    pack["as_of"] = date.today().strftime("%Y-%m")
    pack["fetched_at"] = date.today().isoformat()

    if args.dry_run:
        print(f"  (dry-run) would set: {', '.join(changed)} · regime={pack['regime']}; not written.")
        return 0
...
def main() -> int:
    ...
    ap.add_argument("--version", default="2026", help="Pack version selecting data/rates_<version>.json")
    ap.add_argument("--repo-rate", type=float, default=None, help="latest policy repo rate, %")
    ap.add_argument("--prev-repo-rate", type=float, default=None, help="previous repo rate (default: current pack value)")
    ap.add_argument("--stance", default=None, help="MPC stance: accommodative | neutral | hawkish | …")
    ap.add_argument("--cpi", type=float, default=None, help="set CPI YoY %% directly")
    ap.add_argument("--cpi-resource", default=None, help="data.gov.in OGD resource id for a CPI series")
