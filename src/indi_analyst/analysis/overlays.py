...
class OverlaySpec:
    ...
    direction: int  # +1 when a higher number helps sensitive sectors, -1 when it hurts (input costs)
    ...
    scale_attr: str  # Settings float attr: the (value-neutral) that maps to a full ±1, e.g. "iip_scale"
...
SENSITIVITY_OVERLAYS: list[OverlaySpec] = [
    OverlaySpec(
        kind="iip", label="Industrial output (IIP)", unit="% YoY", direction=1,
        rising_word="expanding", falling_word="contracting", exposure_word="is cyclical",
        version_attr="iip_pack_version", enabled_attr="iip_enabled", path_attr="iip_data_path",
        scale_attr="iip_scale", max_attr="iip_max_points",
    ),
    OverlaySpec(
        kind="gst", label="GST collections", unit="% YoY", direction=1,
        rising_word="rising", falling_word="softening", exposure_word="is consumption-led",
        version_attr="gst_pack_version", enabled_attr="gst_enabled", path_attr="gst_data_path",
        scale_attr="gst_scale", max_attr="gst_max_points",
    ),
    OverlaySpec(
        kind="credit", label="Bank credit growth", unit="% YoY", direction=1,
        rising_word="accelerating", falling_word="slowing", exposure_word="is credit-driven",
        version_attr="credit_pack_version", enabled_attr="credit_enabled", path_attr="credit_data_path",
        scale_attr="credit_scale", max_attr="credit_max_points",
    ),
    OverlaySpec(
        kind="trade", label="Merchandise exports", unit="% YoY", direction=1,
        rising_word="growing", falling_word="declining", exposure_word="is export-oriented",
        version_attr="trade_pack_version", enabled_attr="trade_enabled", path_attr="trade_data_path",
        scale_attr="trade_scale", max_attr="trade_max_points",
    ),
    OverlaySpec(
        kind="inputcost", label="Input-cost inflation (WPI)", unit="% YoY", direction=-1,
        rising_word="rising", falling_word="easing", exposure_word="is input-cost-sensitive",
        version_attr="inputcost_pack_version", enabled_attr="inputcost_enabled", path_attr="inputcost_data_path",
        scale_attr="inputcost_scale", max_attr="inputcost_max_points",
    ),
    OverlaySpec(
        kind="monsoon", label="Monsoon rainfall", unit="% vs LPA", direction=1,
        rising_word="above-normal", falling_word="deficient", exposure_word="is rural-demand-led",
        version_attr="monsoon_pack_version", enabled_attr="monsoon_enabled", path_attr="monsoon_data_path",
        scale_attr="monsoon_scale", max_attr="monsoon_max_points",
    ),
]
