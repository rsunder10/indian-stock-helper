...
def test_parser_counts_paying_years_and_last_dividend():
    ...
    ca = _corporate_actions(
        frame, as_of=date(2024, 12, 31), lookback_years=6, split_recency_days=365
    )
...
def test_parser_returns_none_for_empty_history():
    assert (
        _corporate_actions(pd.DataFrame(), as_of=None, lookback_years=6, split_recency_days=365)
        is None
    )
...
def test_snapshot_pulls_corporate_actions_and_warns_on_recent_split():
    ...
    ca = CorporateActions(
        dividend_paying_years=4,
        lookback_years=6,
        last_split_ratio="2:1",
        last_split_date=date(2024, 6, 1),
        recent_split=True,
    )
