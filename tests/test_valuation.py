...
def test_graham_and_earnings_blend():
    ...
    assert isclose(val.fair_value, round((round(graham, 2) + round(earnings, 2)) / 2, 2), abs_tol=0.05)
...
def test_dividend_skipped_when_r_minus_g_unstable():
    ...
    val = compute_valuation(snap, Settings(fair_value_discount_rate=0.06, fair_value_terminal_growth=0.05))
...
def test_end_to_end_recommendation_has_valuation():
    fund = Fundamentals(pe_ratio=20.0, pb_ratio=3.0, earnings_growth=0.16, dividend_yield=0.012, sector="Testing")
